import useMobile from '@/hooks/client/useMobile';
import { cn } from '@/lib/utils';
import Image from 'next/image';
import JumbotronPNG from '../../../../public/image/jumbotron.png';

const JumbotronImage = () => {
  const { isMobileQuery: isMobileMax1024 } = useMobile('(max-width:1024px)');
  const { isMobileQuery: isMobileMax1680 } = useMobile('(max-width:1680px) and (min-width:1441px)');
  const { isMobileQuery: isMobileMax1440 } = useMobile('(max-width:1440px) and (min-width:1025px)');
  const px1680 = isMobileMax1680 && 'top-[5rem]';
  const px1440 = isMobileMax1440 && 'top-[0]';
  const px1024 = isMobileMax1024 && 'top-[5rem]';
  const topSpace = px1680 || px1440 || px1024 || 'top-[10rem]';
  return (
    <div className={cn('absolute left-[50%] translate-x-[-50%] w-[100%] max-w-[138rem] h-[100vh]', topSpace)}>
      <div className={cn('w-[calc(100%-30rem)] mx-auto h-[auto] rounded-[15rem] overflow-hidden')}>
        <p className={cn('w-[128rem] overflow-hidden')}>
          <Image src={JumbotronPNG} alt="점보트론 이미지" />
        </p>
      </div>
    </div>
  );
};

export default JumbotronImage;
